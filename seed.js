...
async function seedDatabase() {
  ...
  for (let i = 1; i <= 1000; i++) {
    ...
    const [result] = await pool.execute(
      "INSERT IGNORE INTO doctors (name, email, password, specialization) VALUES (?, ?, ?, ?)",
      [name, email, hashedPassword, spec],
    );
  ...
  for (let i = 1; i <= 1000; i++) {
    ...
    await pool.execute(
      "INSERT IGNORE INTO patients (name, email, password) VALUES (?, ?, ?)",
